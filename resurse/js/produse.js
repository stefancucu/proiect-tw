window.addEventListener("load", function() {
    ...
    pret_input.onchange = function() {
        ...
        document.getElementById("pret-curent").innerHTML = "Pret curent: " + pret_input.value + " RON";
    }
    // Get variables 
    function getVars() {
        var vars = {};
        ...
        let disponibilitate = document.getElementById("cautare-disponibilitate").value;
    ...
    function filter() {
        ...
        for (let i = 0; i < produse.length; i++) {
            ...
            let data = {
                ...
                noutati: new Date(produs.getAttribute("data-aparitie")) > new Date("2019-01-01") ? "true" : "false",
        ...
        for (let i = 0; i < produse_data.length; i++) {
            ...
            if (vars.specificatii != "" && !data.specificatii.includes(vars.specificatii)) {
                match = false;
                console.log("specs")
            }
            if (vars.disponibilitate != "oricare" && data.disponibilitate != vars.disponibilitate) {
                match = false;
                console.log("disponibilitate")
            }
            if (vars.producator.length > 0 && !vars.producator.includes(data.producator)) {
                match = false;
    ...
    // Reset
    function reset() {
        document.getElementById("pret-curent").innerHTML = "Pret curent: " + 10000 + " RON";
        let produse = document.getElementsByClassName("container-produs");
        for (let i = 0; i < produse.length; i++) {
            let produs = produse[i];
            produs.classList.remove("none");
        ...
        if(document.getElementById("cautare-tip").disabled == false) {
            document.getElementById("cautare-tip").value = "";
        }
    ...
    calcMedieBtn.addEventListener("click", calcMedie);
});
